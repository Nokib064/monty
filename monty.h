...
/**
 * struct valu_s - variables -args, file, line content
 * @val: value
 * @ptr: pointer to monty file
 * @line_cont: line content
 * @stack_que: flag change stack <-> queue
 * Description: carries values through the program
 */
typedef struct valu_s
{
	char *val;
	FILE *ptr;
	char *line_cont;
...
int execute(char *line_cont, stack_t **stack, unsigned int count_line, FILE *ptr);
...
void _stack(stack_t **head, unsigned int num_line);
void _queue(stack_t **head, unsigned int num_line);
